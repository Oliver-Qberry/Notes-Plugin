...
const SIDE_PANEL_MAX_WIDTH := 300.0
const SIDE_PANEL_MIN_WIDTH := 170.0
const SIDE_PANEL_WIDTH_RATIO := 0.82

@onready var text_edit: TextEdit = $"MainMargin/VBoxContainer/TextEdit"
@onready var title: Label = $"MainMargin/VBoxContainer/HBoxContainer/Title"
...
func update_title(path: String) -> void:
	...
	title.text = "%s - %s" % [project_name, get_file_name(path)]
...
func get_side_panel_width() -> float:
	...
	return clampf(responsive_width, SIDE_PANEL_MIN_WIDTH, SIDE_PANEL_MAX_WIDTH)
